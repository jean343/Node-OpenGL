...
bool GfxTexture::Create(int width, int height, const void* data, bool isColor) {
  Width = width;
  Height = height;
  glGenTextures(1, &Id);
  check();
...
void GfxTexture::SetPixels(const void* data, bool isColor) {
  glBindTexture(GL_TEXTURE_2D, Id);
  check();
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, Width, Height, isColor ? GL_RGBA : GL_LUMINANCE, GL_UNSIGNED_BYTE, data);
  check();
  glBindTexture(GL_TEXTURE_2D, 0);
  check();
}
